...
if exist ".git" (
  ...
  if not errorlevel 1 (
    git pull --ff-only
    if errorlevel 1 echo Hinweis: Git-Update konnte nicht automatisch eingespielt werden.
  )
)
...
echo [3/4] Starte Windows Spider Agent...
echo [4/4] Browser: http://127.0.0.1:8010
